    ...
    public class EmailService : IEmailService
    {
        ...
        public async Task ConnectAndDownloadAllEmailsAsync(EmailServerInfo emailServerInfo, CancellationToken cancellationToken)
        {
            ...
            for (int i = 0; i < ConcurrentConnections; i++)
            {
                ...
                clients.Add(client);
                tasks.Add(ConnectClientAndDownloadAllEmails(client, emailServerInfo, cancellationToken));
            }
        ...
        private async Task ConnectClientAndDownloadAllEmails(EmailClientBase client, EmailServerInfo emailServerInfo, CancellationToken cancellationToken)
        {
            await client.ConnectAsync(emailServerInfo);

            await DownloadAllEmailIDs(client);
            await Task.Run(() => DownloadAllEmailHeaders(client, cancellationToken));
            await Task.Run(() => DownloadEmailBodies(client, cancellationToken));
        }

        private async Task DownloadAllEmailIDs(EmailClientBase client)
        {
        ...
        private async Task DownloadAllEmailHeaders(EmailClientBase client, CancellationToken cancellationToken)
        {
        ...
        private async Task DownloadEmailBodies(EmailClientBase client, CancellationToken cancellationToken)
        {
